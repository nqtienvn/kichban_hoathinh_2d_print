...
## Develop and approve the story before building the batch

After intake and before writing narration, scene timings, or video prompts, run a short story workshop and stop for the user's choice.

- If the user supplied no concept, create 3 distinct story directions grounded in the product and one audience situation.
- If the user supplied a rough idea or notes, treat them as the creative seed and develop 3 distinct versions that preserve the intended meaning while improving the hook, tension, reveal, and ending.
- For each direction, show a short title, opening hook, audience situation or problem, central question/tension, supported product reveal or action, memorable visual turn, ending/next step, and any material assumption. Keep verified product facts separate from creative invention.
- Recommend the strongest direction and explain the choice in one sentence, but do not write prompts, final narration, scene timings, or the batch in the same turn. Wait for the user to select one, combine options, or explicitly ask you to choose.
- Once the user selects a direction, lock its audience, central promise, reveal, and ending before building the batch. If the user explicitly asks to skip brainstorming or requests prompts only, honor that instruction and state that the workshop was skipped.
...
2. Use the selected story direction's audience situation and central product value, grounded in the supplied facts. Follow the storytelling guidance in the reference: immediate visual/spoken hook → small question or tension → timely reveal through a supported product detail/action → practical relevance → a natural next step that resolves or recalls the opening. Scale this arc to N; for a single short clip, compress it to one action and its resolution. Longer videos add distinct supported use situations, not repeated claims or a longer introduction.
